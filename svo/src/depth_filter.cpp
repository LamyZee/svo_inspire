...
// Modify by LamyZee, add seeds tau calc.
// Add Heyijia codes, part of seed depth init of svo_edgelet.
...
void DepthFilter::addFrame(FramePtr frame) {
  if (thread_ != nullptr) {
    ...
    // TODO(LamyZee): need to test this threshold
...
void DepthFilter::addKeyframe(FramePtr frame,
                              double depth_mean,
                              double depth_min) {
...
void DepthFilter::updateSeeds(FramePtr frame) {
 ...
 // law of chord (sehnensatz)  
  ...
  while (it!=seeds_.end()) {
    ...
    if ((Seed::batch_counter - it->batch_id) > 5) {
      it = seeds_.erase(it);
      continue;
    }
    ...
    const Vector3d xyz_f(T_ref_cur.inverse() * (1.f / it->mu * it->ftr->f));
    if (xyz_f.z() < 0.0) {
    ...
    if (!matcher_.findEpipolarMatchDirect(
                  *it->ftr->frame, *frame, *it->ftr, 1.f / it->mu,
                  1.f / z_inv_min, 1.f / z_inv_max, z, tau_match)) {
    ...
    // compute tau
    double tau = computeTau(T_ref_cur, it->ftr->f, z,
                            px_error_angle * tau_match);
    double tau_inverse = 0.5 * (1.f / max(0.0000001, z - tau)
                       - 1.f / (z + tau));
    // update the estimate
...
void DepthFilter::getSeedsCopy(const FramePtr& frame, std::list<Seed>& seeds) {
  lock_t lock(seeds_mut_);
  for (std::list<Seed>::iterator it = seeds_.begin();
       it != seeds_.end(); ++it) {
    if (it->ftr->frame == frame.get())
      seeds.push_back(*it);
  }
...
void DepthFilter::updateSeed(const float x, const float tau2, Seed* seed) {
  ...
  float s2 = 1.f / (1.f / seed->sigma2 + 1.f / tau2);
  float m = s2 * (seed->mu/seed->sigma2 + x / tau2);
  float C1 = seed->a / (seed->a + seed->b) * boost::math::pdf(nd, x);
  float C2 = seed->b / (seed->a + seed->b) * 1.f/seed->z_range;
  float normalization_constant = C1 + C2;
  C1 /= normalization_constant;
  C2 /= normalization_constant;
  float f = C1*(seed->a + 1.f) / (seed->a + seed->b + 1.f)
          + C2*seed->a / (seed->a + seed->b + 1.f);
  float e = C1*(seed->a + 1.f) * (seed->a + 2.f)
          / ((seed->a + seed->b + 1.f) * (seed->a + seed->b + 2.f))
          + C2 * seed->a * (seed->a + 1.f)
          / ((seed->a + seed->b + 1.f) * (seed->a + seed->b + 2.0f));
  // update parameters
  float mu_new = C1 * m + C2 * seed->mu;
  seed->sigma2 = C1 * (s2 + m * m)
               + C2 * (seed->sigma2 + seed->mu * seed->mu)
               - mu_new * mu_new;
  seed->mu = mu_new;
  seed->a = (e - f) / (f - e / f);
